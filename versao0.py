...
def girar_peca(peca, grade):
    peca.rotacao = (peca.rotacao + 1) % len(peca.formato)
    if not espaco_valido(peca, grade):
        peca.rotacao = (peca.rotacao - 1) % len(peca.formato)
    ...
    while rodando:
        ...
        for evento in pygame.event.get():
            ...
            if evento.type == pygame.KEYDOWN:
                ...
                elif evento.key == pygame.K_RETURN:  # Girar peça ao pressionar Enter
                    girar_peca(peca_atual, grade)
...
class Jogo:
    def __init__(self):
        self.posicoes_travadas = {}
        self.grade = criar_grade(self.posicoes_travadas)
        self.peca_atual = obter_peca()
        self.proxima_peca = obter_peca()
        self.pontuacao = 0
        self.rodando = True
    ...
    def aumentar_pontuacao(self, linhas):
        self.pontuacao += linhas * 100

def menu_principal():
    rodando = True
    while rodando:
        janela.fill((0, 0, 0))
        desenhar_texto_centralizado('Pressione qualquer tecla para começar.', 20, (255, 255, 255), janela)
        pygame.display.update()
        for evento in pygame.event.get():
            if evento.type == pygame.QUIT:
                rodando = False
